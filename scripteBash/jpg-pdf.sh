...
while read ORDNER ; do 
	...
	if [ $exist -ge 1 ]; then
  		ls *.jpg | cat -n | while read zaehler file; do 
								mv "$file" "$ORDNER-$zaehler.jpg"; 
							done
	fi
	EXTENSION="png" 
	exist=$(find -iname "*.$EXTENSION" | wc -l)
	if [ $exist -ge 1 ]; then
		ls *.png | cat -n | while read zaehler file; do 
								mv "$file" "$ORDNER-$zaehler.png"; 
							done
	fi
	...
	for filename in *.eps; do
		...
		# win10
		gswin64.exe -dEPSCrop -dBATCH -dNOPAUSE -sOutputFile=$pdfname -sDEVICE=pdfwrite \
			-f $filename
		# ubuntu
